...
import {
  ...
  getOwnershipDocType,
  validateAssetStep,
} from '../../assets/utils/asset-form-utils.js';
...
export function CreateAuctionModal({ open, onClose, onSuccess, initialAssetId = null }) {
  ...
  const [assetCreationMode, setAssetCreationMode] = useState('select'); // 'select' or 'create'
  const [newAssetForm, setNewAssetForm] = useState(buildEmptyAssetForm);
  const [newAssetImagePreviews, setNewAssetImagePreviews] = useState([]);
  const [creatingAsset, setCreatingAsset] = useState(false);

  const imageInputRef = useRef(null);
  const documentInputRef = useRef(null);
  const newAssetImageInputRef = useRef(null);
  const newAssetOwnershipDocInputRef = useRef(null);
  const newAssetAdditionalDocInputRef = useRef(null);
  ...
  useEffect(() => {
    if (!open) {
      ...
      // Reset new asset state
      setAssetCreationMode('select');
      setNewAssetForm(buildEmptyAssetForm());
      setNewAssetImagePreviews((current) => {
        current.forEach((preview) => {
          if (preview?.url) {
            URL.revokeObjectURL(preview.url);
          }
  ...
  // --- New asset creation helpers ---
  const updateNewAssetField = (field, value) => {
    setNewAssetForm((current) => ({ ...current, [field]: value }));
    clearFieldError(field);
  };

  const handleNewAssetImageSelect = (event) => {
    const selected = Array.from(event.target.files || []).filter(
      (file) => file.type.startsWith('image/'),
    );
    if (!selected.length) return;

    const nextImages = [...newAssetForm.photoFiles, ...selected];
    const nextPreviews = [
      ...newAssetImagePreviews,
      ...selected.map((file) => ({
        id: `${file.name}-${file.size}-${file.lastModified}`,
        url: URL.createObjectURL(file),
        name: file.name,
    ...
    setNewAssetForm((current) => ({ ...current, photoFiles: nextImages }));
    setNewAssetImagePreviews(nextPreviews);
    clearFieldError('photos');
    event.target.value = '';
  };

  const removeNewAssetImage = (index) => {
  ...
  const handleNewAssetDocumentSelect = (event, isOwnership) => {
    ...
    if (isOwnership) {
      // For ownership document, we only store the file temporarily to upload
      updateNewAssetField('ownershipDocumentFile', selected[0]);
    } else {
      const nextDocs = [
        ...newAssetForm.additionalDocuments,
        ...selected.map((file) => ({
          id: `${file.name}-${file.size}-${file.lastModified}`,
          file,
          name: file.name,
      ...
      setNewAssetForm((current) => ({ ...current, additionalDocuments: nextDocs }));
    }
    clearFieldError(isOwnership ? 'ownershipDocumentUrl' : 'additionalDocuments');
    event.target.value = '';
  };

  const removeNewAssetDocument = (id) => {
    setNewAssetForm((current) => ({
      ...current,
      additionalDocuments: current.additionalDocuments.filter((doc) => doc.id !== id),
    }));
  };

  const handleCreateNewAsset = async () => {
    // Validate the new asset form
    const stepsToValidate = ['details', 'location', 'photos', 'documents'];
    let validationErrors = {};
    stepsToValidate.forEach(step => {
      validationErrors = { ...validationErrors, ...validateAssetStep(step, newAssetForm, t) };
    });
    ...
    try {
      ...
      const response = await assetService.staffCreate({
        userId: user.id, // Use current staff user as owner
        ...assetPayload,
      });

      const newAsset = response.asset;

      // 4. Add the new asset to eligible assets (in case we need to select it)
      setEligibleAssets([newAsset, ...eligibleAssets]);

      // 5. Select the new asset (only single mode allowed for new assets)
      setAuctionMode(AUCTION_MODES.SINGLE);
      const newLot = buildSelectedLotFromAsset(newAsset, 0);
      setSelectedLots([newLot]);
      applyAssetPrefill(newAsset);

      // 6. Switch back to select mode and proceed to next step
      setAssetCreationMode('select');
      goToStep(AUCTION_STEPS.BASIC);
    } catch (err) {
    ...
    <div className="kyc-modal-overlay" role="presentation" onClick={handleOverlayClick}>
      ...
      >
        ...
        <div className="auction-create-modal__body">
          {step === AUCTION_STEPS.ASSET && (
            <div className="auction-create-modal__asset-step">
              ...
              <fieldset className="auction-create-modal__mode-toggle" style={{ marginBottom: '16px' }}>
                <legend className="input-field__label">How would you like to add an asset?</legend>
                <label className="auction-create-modal__mode-option">
                  <input
                    type="radio"
                    name="asset-creation-mode"
                    value="select"
                    checked={assetCreationMode === 'select'}
                    onChange={() => setAssetCreationMode('select')}
                    disabled={submitting || creatingAsset}
                  />
                ...
                <label className="auction-create-modal__mode-option">
                  <input
                    ...
                    name="asset-creation-mode"
                    value="create"
                    checked={assetCreationMode === 'create'}
                    onChange={() => {
                      setAssetCreationMode('create');
                      // When creating new asset, force single mode
                      setAuctionMode(AUCTION_MODES.SINGLE);
                    }}
                    disabled={submitting || creatingAsset}
                  />
              ...
              {/* Auction mode toggle - only visible when selecting existing */}
              {assetCreationMode === 'select' && (
              ...
              {/* CREATE NEW ASSET UI */}
              {assetCreationMode === 'create' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  <p className="auction-create-modal__section-hint">
                    Create a new asset to add to this auction. The asset will be marked as evaluated immediately.
                  </p>
                  ...
                  <div className="auction-create-modal__grid">
                    <Input
                      label="Asset Title"
                      name="new-asset-title"
                      value={newAssetForm.title}
                      onChange={(e) => updateNewAssetField('title', e.target.value)}
                      error={errors.title}
                      disabled={submitting || creatingAsset}
                    />
                    <div className="input-field">
                      <label className="input-field__label" htmlFor="new-asset-type">Asset Type</label>
                      <select
                        id="new-asset-type"
                        className={[
                        ...
                        onChange={(e) => {
                          ...
                          updateNewAssetField('assetType', type);
                          // Update ownership document type to match asset type
                          if (type) {
                            updateNewAssetField('ownershipDocumentType', getOwnershipDocType(type));
                          }
                        }}
                        disabled={submitting || creatingAsset}
                      ...
                      >
                        ...
                        {ASSET_TYPE_KEYS.map((key) => (
                          <option key={key} value={key}>
                            {t(`assets.types.${key}`, { defaultValue: key })}
                          </option>
                        ))}
                      </select>
                      {errors.assetType && (
                        <span className="input-field__error" role="alert">{errors.assetType}</span>
                      )}
                    </div>
                  </div>

                  <div className="input-field auction-create-modal__full">
                    <label className="input-field__label" htmlFor="new-asset-description">Description</label>
                    <textarea
                      id="new-asset-description"
                      className="kyc-modal__textarea auction-create-modal__textarea"
                      rows={3}
                      value={newAssetForm.description}
                      onChange={(e) => updateNewAssetField('description', e.target.value)}
                      disabled={submitting || creatingAsset}
                    />
                    {errors.description && (
                      <span className="input-field__error" role="alert">{errors.description}</span>
                    )}
                  </div>

                  <div className="input-field auction-create-modal__full">
                    <label className="input-field__label" htmlFor="new-asset-condition">Condition Notes</label>
                    <textarea
                      id="new-asset-condition"
                      className="kyc-modal__textarea auction-create-modal__textarea"
                      rows={2}
                      value={newAssetForm.conditionNotes}
                      onChange={(e) => updateNewAssetField('conditionNotes', e.target.value)}
                      disabled={submitting || creatingAsset}
                    />
                    {errors.conditionNotes && (
                      <span className="input-field__error" role="alert">{errors.conditionNotes}</span>
                    )}
                  </div>
                  ...
                  <div className="auction-create-modal__grid">
                    <Input
                      label="Location"
                      name="new-asset-location"
                      value={newAssetForm.location}
                      onChange={(e) => updateNewAssetField('location', e.target.value)}
                      error={errors.location}
                      disabled={submitting || creatingAsset}
                    />
                    <Input
                      label="Address (Optional)"
                      name="new-asset-address"
                      value={newAssetForm.address}
                      onChange={(e) => updateNewAssetField('address', e.target.value)}
                      disabled={submitting || creatingAsset}
                    />
                  </div>
                  ...
                  <div className="auction-create-modal__grid">
                    <Input
                      label="Desired Reserve Price"
                      name="new-asset-reserve"
                      type="number"
                      min="0"
                      step="0.01"
                      value={newAssetForm.desiredReservePrice}
                      onChange={(e) => updateNewAssetField('desiredReservePrice', e.target.value)}
                  ...
                  <div className="input-field auction-create-modal__full">
                    <label className="input-field__label" htmlFor="new-asset-conditions">Auction Conditions</label>
                    <textarea
                      id="new-asset-conditions"
                      className="kyc-modal__textarea auction-create-modal__textarea"
                      rows={3}
                      value={newAssetForm.auctionConditions}
                      onChange={(e) => updateNewAssetField('auctionConditions', e.target.value)}
                      disabled={submitting || creatingAsset}
                    />
                    {errors.auctionConditions && (
                      <span className="input-field__error" role="alert">{errors.auctionConditions}</span>
                    )}
                  </div>

                  {/* Photos */}
                  <section className="auction-create-modal__upload-section">
                    <h3 className="auction-create-modal__section-title">Photos</h3>
                    <p className="auction-create-modal__section-hint">
                    ...
                    <div
                      ...
                      onDrop={(event) => {
                        ...
                        handleNewAssetImageSelect(syntheticEvent);
                      }}
                    >
                      ...
                      <button
                        ...
                        onClick={() => newAssetImageInputRef.current?.click()}
                        disabled={submitting || creatingAsset}
                      >
                        Select Photos
                      </button>
                      <input
                        ref={newAssetImageInputRef}
                        type="file"
                        accept={IMAGE_ACCEPT}
                        multiple
                        hidden
                        onChange={handleNewAssetImageSelect}
                      />
                    </div>

                    {newAssetImagePreviews.length > 0 && (
                      <div className="auction-create-modal__image-grid">
                        {newAssetImagePreviews.map((preview, index) => (
                          <div key={preview.id} className="auction-create-modal__image-card">
                            <img src={preview.url} alt={preview.name} />
                            <button
                              type="button"
                              className="auction-create-modal__remove-btn"
                              onClick={() => removeNewAssetImage(index)}
                              disabled={submitting || creatingAsset}
                    ...
                    {errors.photos && (
                      <p className="kyc-modal__error" role="alert">{errors.photos}</p>
                    )}
                  </section>

                  {/* Documents */}
                  <section className="auction-create-modal__upload-section">
                    <h3 className="auction-create-modal__section-title">Ownership Document</h3>
                    <p className="auction-create-modal__section-hint">
                      Upload the ownership document for this asset
                    </p>
                    <div
                      className="auction-create-modal__dropzone"
                      onDragOver={(event) => event.preventDefault()}
                    ...
                    >
                      ...
                      <button
                        type="button"
                        className="auction-create-modal__browse-btn"
                        onClick={() => newAssetOwnershipDocInputRef.current?.click()}
                        disabled={submitting || creatingAsset}
                      >
                    ...
                    {newAssetForm.ownershipDocumentFile && (
                      <ul className="auction-create-modal__doc-list">
                        <li className="auction-create-modal__doc-item">
                          <div>
                            <p className="auction-create-modal__doc-name">
                              {newAssetForm.ownershipDocumentFile.name}
                            </p>
                            <p className="auction-create-modal__doc-size">
                              {formatFileSize(newAssetForm.ownershipDocumentFile.size)}
                            </p>
                          </div>
                          <button
                            type="button"
                            className="auction-create-modal__remove-btn"
                            onClick={() => updateNewAssetField('ownershipDocumentFile', null)}
                            disabled={submitting || creatingAsset}
                          >
                            ×
                          </button>
                    ...
                    {errors.ownershipDocumentUrl && (
                      <p className="kyc-modal__error" role="alert">{errors.ownershipDocumentUrl}</p>
                    )}
                  </section>

                  <section className="auction-create-modal__upload-section">
                    <h3 className="auction-create-modal__section-title">Additional Documents</h3>
                    <p className="auction-create-modal__section-hint">
                      Upload any additional supporting documents (PDFs)
                    </p>
                    <div
                      className="auction-create-modal__dropzone"
                      onDragOver={(event) => event.preventDefault()}
                      onDrop={(event) => {
                        ...
                        if (!files.length) return;
                        const syntheticEvent = { target: { files, value: '' } };
                        handleNewAssetDocumentSelect(syntheticEvent, false);
                      }}
                    >
                      ...
                      <button
                        type="button"
                        className="auction-create-modal__browse-btn"
                        onClick={() => newAssetAdditionalDocInputRef.current?.click()}
                        disabled={submitting || creatingAsset}
                      >
                        Select Additional Documents
                      </button>
                      <input
                        ref={newAssetAdditionalDocInputRef}
                        type="file"
                        accept={PDF_ACCEPT}
                        multiple
                        hidden
                        onChange={(e) => handleNewAssetDocumentSelect(e, false)}
                      />
                    </div>
                    {newAssetForm.additionalDocuments.length > 0 && (
                      <ul className="auction-create-modal__doc-list">
                        {newAssetForm.additionalDocuments.map((doc) => (
                          <li key={doc.id} className="auction-create-modal__doc-item">
                            <div>
                              <p className="auction-create-modal__doc-name">{doc.name}</p>
                              <p className="auction-create-modal__doc-size">{formatFileSize(doc.size)}</p>
                            </div>
                            <button
                              type="button"
                              className="auction-create-modal__remove-btn"
                              onClick={() => removeNewAssetDocument(doc.id)}
                              disabled={submitting || creatingAsset}
                            >
                              ×
                            </button>
                    ...
                    {errors.additionalDocuments && (
                      <p className="kyc-modal__error" role="alert">{errors.additionalDocuments}</p>
                    )}
                  </section>
                </div>
              )}
        ...
        <div className="kyc-modal__actions auction-create-modal__actions">
          {/* Show back button if not on first step OR creating asset */}
          {(stepIndex > 0 || (step === AUCTION_STEPS.ASSET && assetCreationMode === 'create')) && (
            <Button
              variant="secondary"
              onClick={() => {
                if (step === AUCTION_STEPS.ASSET && assetCreationMode === 'create') {
                  setAssetCreationMode('select');
                  setErrors({});
                } else {
                  handleBack();
              ...
              disabled={submitting || creatingAsset}
            >
              {step === AUCTION_STEPS.ASSET && assetCreationMode === 'create'
                ? 'Back to Select'
          ...
          <div className="auction-create-modal__actions-right">
            <Button variant="secondary" onClick={onClose} disabled={submitting || creatingAsset}>
              {t('auctions.create.actions.cancel')}
            </Button>

            {/* Handle buttons for asset step in create mode */}
            {step === AUCTION_STEPS.ASSET && assetCreationMode === 'create' ? (
              <Button
                variant="primary"
                ...
                disabled={submitting || creatingAsset}
              >
                {creatingAsset ? 'Creating Asset...' : 'Create Asset & Continue'}
              </Button>
            ) : (
              <>
                {step !== AUCTION_STEPS.REVIEW ? (
